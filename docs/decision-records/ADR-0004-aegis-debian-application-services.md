...
The applications are separate private monorepos and must not share a release, guest runtime, database, service identity, or guest-level failure domain. Each service therefore requires its own Debian 12 virtual machine. Shared host and infrastructure dependencies remain explicit below. The VM Factory remains plan-only: apply is disabled, the image catalog is empty, the required application-network profiles are absent, and only the `none` bootstrap profile is approved. This decision records intent without authorizing a Hyper-V, OPNsense, DNS, SMTP, Wazuh, TacticalRMM, backup, guest, or application change.

This decision extends [ADR-0003](ADR-0003-segmented-windows-workstation-fleet.md)'s proposed single private Hyper-V trunk fabric with access VLANs 150 and 160. The extension requires a separate guarded control-plane change, updated immutable switch/trunk policy, OPNsense configuration backup, and new negative tests; it does not amend the live trunk or authorize routine VM provisioning to create or rebind network infrastructure.
