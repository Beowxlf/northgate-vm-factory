...
The owner also authorized VLAN 150 (`BUSINESS-APPS`) and VLAN 160 (`COMMERCIAL-DMZ`) as target network design. A separately controlled live change has now created the private Hyper-V fabric `NorthGate-App-Trunk`, attached only the dedicated OPNsense `APP-TRUNK` adapter, and activated the two OPNsense VLAN gateways. No workload adapter is connected. DHCP is disabled, no interface pass rule or NAT rule exists for either zone, and existing management remained on the original LAN. The opaque VM catalog profiles remain proposed as `business-apps` and `commercial-dmz`; live fabric existence is not catalog promotion or workload authority. No routine VM action may create, rebind, or alter this fabric.
...
Steps 1-3 were completed and independently read back on 2026-08-02 through the guarded host and OPNsense paths. The live switch is Private with no management-OS binding; only OPNsense is attached; VLANs 150 and 160 terminate on their intended gateways; the loaded OPNsense model validates; DHCP, pass-rule, and NAT counts are zero; and pre/post rollback copies were hashed. Steps 4-5 remain mandatory before any workload connection. Detailed live evidence stays in Operation-SeeSaw rather than this Git repository.
...
| Network | Private `NorthGate-App-Trunk`; dedicated OPNsense `APP-TRUNK`; VLAN 150 and 160 uniqueness; OPNsense backup; gateways, default-deny rules, DNS, and immutable host-policy fingerprints | **Partially satisfied:** private switch, exact trunk, VLAN gateways, routes, default deny, zero DHCP/pass/NAT, management continuity, and rollback copies are live and verified. **Still blocked:** no workload access port, DNS/service reservation, immutable installed host-policy mapping, negative client test, or promoted opaque network profile exists |
...
## Explicit no-workload-change result

The strict proposal records the owner's selected ISO source, VLAN targets, resource envelopes, and proposed profile set without widening `applyEnabled: false` or adding executable actions. It does not convert source-media authorization into bulk image promotion. Separate live preparation has completed the isolated VLAN control plane, but no workload adapter, VM, manifest, image promotion, or host-issued plan exists. Detailed evidence remains outside this repository and grants no workload authority.
